...
std::shared_ptr<TriangleMesh> TetraMesh::ExtractTriangleMesh(
    ...
    for (size_t tetra_i = 0; tetra_i < tetras_.size(); ++tetra_i) {
        ...
        for (auto tetra_edge : tetra_edges) {
            Index edge_vert1 = tetra[tetra_edge[0]];
            Index edge_vert2 = tetra[tetra_edge[1]];
        ...
        for (int i = 0; i < num_verts; ++i) {
            if (intersecting_edges.count(keys[i]) == 0) {
                auto idx = static_cast<Index>(intersecting_edges.size());
                verts_indices[i] = idx;
