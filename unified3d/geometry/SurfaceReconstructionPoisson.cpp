...
public:
    Open3DPointStream(const u3d::geometry::PointCloud* pcd)
...
public:
    const u3d::geometry::PointCloud* pcd_;
    XForm<Real, 4>* xform_;
    size_t current_;
};
...
void ExtractMesh(
        ...
        std::shared_ptr<u3d::geometry::TriangleMesh>& out_mesh,
    ...
    for (size_t tidx = 0; tidx < mesh->polygonCount(); ++tidx) {
        ...
        if (triangle.size() != 3) {
            u3d::utility::LogError("got polygon");
        } else {
...
void Execute(const u3d::geometry::PointCloud& pcd,
             std::shared_ptr<u3d::geometry::TriangleMesh>& out_mesh,
             std::vector<double>& out_densities,
             int depth,
             float width,
