...
#include "unified3d/geometry/MeshBase.h"
    ...
    public:
        ...
        HalfEdge()
            : next_(-1),
              ...
              vertex_indices_(-1, -1),
              triangle_index_(-1) {}
        HalfEdge(Eigen::Vector2i vertex_indices,
                 int triangle_index,
                 int next,
                 int twin);
...
public:
    ...
    [[nodiscard]] std::vector<int> BoundaryHalfEdgesFromVertex(
            int vertex_index) const;

    /// Query manifold boundary vertices from a starting vertex
    /// If query vertex is not on boundary, empty vector will be returned.
    [[nodiscard]] std::vector<int> BoundaryVerticesFromVertex(
            int vertex_index) const;
