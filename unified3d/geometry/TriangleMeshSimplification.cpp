...
public:
    ...
    [[nodiscard]] double Eval(const Eigen::Vector3d& v) const {
    ...
    [[nodiscard]] bool IsInvertible() const {
        return std::fabs(A_.determinant()) > 1e-4;
    }

    [[nodiscard]] Eigen::Vector3d Minimum() const {
...
std::shared_ptr<TriangleMesh> TriangleMesh::SimplifyVertexClustering(
    ...
    for (const auto& triangle : triangles_) {
        ...
        triangles.emplace(vidx0, vidx1, vidx2);
    }
