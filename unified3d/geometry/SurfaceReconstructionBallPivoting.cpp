...
#include "unified3d/geometry/IntersectionTest.h"
...
public:
    ...
    void AddAdjacentTriangle(const BallPivotingTrianglePtr& triangle);
    [[nodiscard]] BallPivotingVertexPtr GetOppositeVertex() const;
...
public:
    BallPivotingTriangle(BallPivotingVertexPtr vert0,
        ...
        : vert0_(vert0),
          vert1_(vert1),
          vert2_(vert2),
          ball_center_(std::move(ball_center)) {}
...
void BallPivotingEdge::AddAdjacentTriangle(
        const BallPivotingTrianglePtr& triangle) {
...
BallPivotingVertexPtr BallPivotingEdge::GetOppositeVertex() const {
    if (triangle0_ != nullptr) {
...
public:
    ...
    BallPivotingEdgePtr GetLinkingEdge(const BallPivotingVertexPtr& v0,
        ...
        for (BallPivotingEdgePtr edge0 : v0->edges_) {
            for (const BallPivotingEdgePtr& edge1 : v1->edges_) {
    ...
    void CreateTriangle(const BallPivotingVertexPtr& v0,
        ...
        if (face_normal.dot(v0->normal_) > -1e-16) {
            mesh_->triangles_.emplace_back(v0->idx_, v1->idx_, v2->idx_);
        } else {
            mesh_->triangles_.emplace_back(v0->idx_, v2->idx_, v1->idx_);
        }
