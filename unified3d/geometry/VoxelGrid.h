...
public:
    ...
    Voxel(Eigen::Vector3i grid_index) : grid_index_(std::move(grid_index)) {}
    /// \brief Parameterized Constructor.
    ///
    /// \param grid_index Grid coordinate index of the voxel.
...
public:
    ...
    [[nodiscard]] Eigen::Vector3d GetVoxelCenterCoordinate(
    ...
    [[nodiscard]] std::vector<Eigen::Vector3d> GetVoxelBoundingPoints(
    ...
    [[nodiscard]] std::vector<bool> CheckIfIncluded(
            const std::vector<Eigen::Vector3d> &queries) const;
    ...
    [[nodiscard]] std::shared_ptr<geometry::Octree> ToOctree(
