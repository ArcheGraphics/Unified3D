...
#include "unified3d/utility/IJsonConvertible.h"
...
public:
    ...
    [[nodiscard]] std::pair<double, double> GetFocalLength() const {
    ...
    /// point.
    [[nodiscard]] std::pair<double, double> GetPrincipalPoint() const {
        return std::make_pair(intrinsic_matrix_(0, 2), intrinsic_matrix_(1, 2));
    }

    /// Returns the skew.
    [[nodiscard]] double GetSkew() const { return intrinsic_matrix_(0, 1); }

    /// Returns `true` iff both the width and height are greater than 0.
    [[nodiscard]] bool IsValid() const { return (width_ > 0 && height_ > 0); }
