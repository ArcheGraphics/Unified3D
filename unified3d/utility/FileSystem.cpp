...
namespace u3d::utility::filesystem {
...
std::vector<std::string> GetPathComponents(const std::string &path) {
    ...
    auto pathComponents = SplitByPathSeparators(path);
...
std::vector<std::string> FindFilesRecursively(
        ...
        const std::function<bool(const std::string &)> &is_match) {
...
std::string CFile::GetError() const { return GetIOErrorString(error_code_); }
