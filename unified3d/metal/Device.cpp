...
std::pair<MTL::Library*, NS::Error*> load_library_from_path(MTL::Device* device,
                                                            const char* path) {
...
MTL::CommandBuffer* Device::new_command_buffer(int index) {
    ...
    if (qit == queue_map_.end()) {
        throw std::runtime_error(
                "[metal::Device] Attempting to get command buffer for invalid "
                "queue.");
    }
...
MTL::Function* Device::get_function_(const std::string& name,
    ...
    for (auto [value, type, index] : func_consts) {
        mtl_func_consts->setConstantValue(value, (MTL::DataType)type, index);
    }
    ...
    desc->setSpecializedName(NS::String::string(specialized_name.c_str(),
...
MTL::ComputePipelineState* Device::get_kernel_(
        const std::string& name, const MTL::Function* mtl_function) {
