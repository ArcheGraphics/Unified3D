...
using MTLFCList =
        std::vector<std::tuple<const void *, std::uintptr_t, std::uintptr_t>>;
...
public:
    ...
    void register_library(const std::string &lib_name,
                          const std::function<std::string(const std::string &)>
                                  &lib_path_func = get_colocated_mtllib_path);
    ...
    MTL::Function *get_function(const std::string &base_name,
                                MTL::Library *mtl_lib,
...
private:
    ...
    std::unordered_map<int32_t, std::pair<int, MTL::CommandBuffer *>>
            buffer_map_;
