...
public:
    ...
    [[nodiscard]] TensorKeyMode GetMode() const { return mode_; }
...
public:
    ...
    [[nodiscard]] int64_t GetIndex() const { return index_; }
...
public:
    SliceImpl(std::optional<int64_t> start,
        ...
        : TensorKey::Impl(TensorKeyMode::Slice),
          ...
          stop_(stop),
          step_(step) {}
    [[nodiscard]] std::shared_ptr<SliceImpl> InstantiateDimSize(
...
public:
    IndexTensorImpl(Tensor index_tensor)
        : TensorKey::Impl(TensorKeyMode::IndexTensor),
          index_tensor_(std::move(index_tensor)) {}
    [[nodiscard]] Tensor GetIndexTensor() const { return index_tensor_; }
    [[nodiscard]] std::string ToString() const override {
        std::stringstream ss;
