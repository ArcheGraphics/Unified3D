...
public:
    AdvancedIndexPreprocessor(Tensor tensor,
                              const std::vector<Tensor>& index_tensors)
        : tensor_(std::move(tensor)),
          index_tensors_(ExpandBoolTensors(index_tensors)) {
        RunPreprocess();
    }

    [[nodiscard]] inline Tensor GetTensor() const { return tensor_; }

    [[nodiscard]] inline std::vector<Tensor> GetIndexTensors() const {
        return index_tensors_;
    }

    [[nodiscard]] inline SizeVector GetOutputShape() const {
        return output_shape_;
    }

    [[nodiscard]] inline SizeVector GetIndexedShape() const {
        return indexed_shape_;
    }

    [[nodiscard]] inline SizeVector GetIndexedStrides() const {
