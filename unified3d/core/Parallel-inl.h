...
void merge(RandomIterator a,
    ...
    parallelFor(0, size, [&](size_t i) { a[i] = temp[i]; });
}

template <typename RandomIterator,
          typename RandomIterator2,
          typename CompareFunction>
void parallelMergeSort(RandomIterator a,
                       size_t size,
                       RandomIterator2 temp,
...
void parallelFill(const RandomIterator& begin,
    ...
    parallelFor(
            0, size, [begin, value](size_t i) { begin[i] = value; }, policy);
}
