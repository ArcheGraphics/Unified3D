...
public:
    ...
    /// before casting.
    [[nodiscard]] Tensor ReinterpretCast(const core::Dtype& dtype) const;
    ...
    [[nodiscard]] Tensor Reverse() const;
    ...
    /// ```
    [[nodiscard]] Tensor GetItem(const TensorKey& tk) const;
    ...
    /// ```
    ///
    [[nodiscard]] Tensor GetItem(const std::vector<TensorKey>& tks) const;
    ...
    [[nodiscard]] Tensor Append(
            const Tensor& other,
            const std::optional<int64_t>& axis = std::nullopt) const;

    /// Broadcast Tensor to a new broadcastable shape.
    [[nodiscard]] Tensor Broadcast(const SizeVector& dst_shape) const;
    ...
    [[nodiscard]] Tensor Expand(const SizeVector& dst_shape) const;
    ...
    [[nodiscard]] Tensor Reshape(const SizeVector& dst_shape) const;
    ...
    /// - aten/src/ATen/native/TensorShape.cpp
    /// - aten/src/ATen/TensorUtils.cpp
    [[nodiscard]] Tensor View(const SizeVector& dst_shape) const;
    ...
    [[nodiscard]] Tensor To(Dtype dtype, bool copy = false) const;
    ...
    [[nodiscard]] Tensor To(const Device& device, bool copy = false) const;
    ...
    [[nodiscard]] Tensor To(const Device& device,
                            Dtype dtype,
                            bool copy = false) const;

    [[nodiscard]] std::string ToString(bool with_suffix = true,
    ...
    [[nodiscard]] Tensor IndexExtract(int64_t dim, int64_t idx) const;
    ...
    [[nodiscard]] Tensor Slice(int64_t dim,
                               int64_t start,
                               int64_t stop,
    ...
    [[nodiscard]] Tensor AsRvalue() const { return *this; }
    ...
    [[nodiscard]] Tensor IndexGet(
            const std::vector<Tensor>& index_tensors) const;
    ...
    [[nodiscard]] Tensor Add(const Tensor& value) const;
    [[nodiscard]] Tensor Add(Scalar value) const;
    ...
    [[nodiscard]] Tensor Sub(const Tensor& value) const;
    [[nodiscard]] Tensor Sub(Scalar value) const;
    ...
    [[nodiscard]] Tensor Mul(const Tensor& value) const;
    [[nodiscard]] Tensor Mul(Scalar value) const;
    ...
    [[nodiscard]] Tensor Div(const Tensor& value) const;
    [[nodiscard]] Tensor Div(Scalar value) const;
    ...
    [[nodiscard]] Tensor ArgMin(const SizeVector& dims) const;
    ...
    /// the specified dimension. If \p dims contains all dimensions, the index
    /// is into the flattened tensor.
    [[nodiscard]] Tensor ArgMax(const SizeVector& dims) const;
    ...
    /// True values.
    [[nodiscard]] Tensor IsFinite() const;
    ...
    [[nodiscard]] Tensor Clip(Scalar min_val, Scalar max_val) const;
    ...
    [[nodiscard]] Tensor LogicalNot() const;
    ...
    [[nodiscard]] Tensor LogicalAnd(const Tensor& value) const;
    Tensor operator&&(const Tensor& value) const { return LogicalAnd(value); }
    [[nodiscard]] Tensor LogicalAnd(Scalar value) const;
    ...
    /// If the tensor is not boolean, zero will be treated as False, while
    /// non-zero values will be treated as True.
    [[nodiscard]] Tensor LogicalOr(const Tensor& value) const;
    Tensor operator||(const Tensor& value) const { return LogicalOr(value); }
    [[nodiscard]] Tensor LogicalOr(Scalar value) const;
    ...
    /// If the tensor is not boolean, zero will be treated as False, while
    /// non-zero values will be treated as True.
    [[nodiscard]] Tensor LogicalXor(const Tensor& value) const;
    [[nodiscard]] Tensor LogicalXor(Scalar value) const;
    ...
    [[nodiscard]] Tensor Gt(const Tensor& value) const;
    Tensor operator>(const Tensor& value) const { return Gt(value); }
    [[nodiscard]] Tensor Gt(Scalar value) const;
    ...
    [[nodiscard]] Tensor Lt(const Tensor& value) const;
    Tensor operator<(const Tensor& value) const { return Lt(value); }
    [[nodiscard]] Tensor Lt(Scalar value) const;
    ...
    /// boolean tensor.
    [[nodiscard]] Tensor Ge(const Tensor& value) const;
    Tensor operator>=(const Tensor& value) const { return Ge(value); }
    [[nodiscard]] Tensor Ge(Scalar value) const;
    ...
    /// tensor.
    [[nodiscard]] Tensor Le(const Tensor& value) const;
    Tensor operator<=(const Tensor& value) const { return Le(value); }
    [[nodiscard]] Tensor Le(Scalar value) const;
    ...
    [[nodiscard]] Tensor Eq(const Tensor& value) const;
    Tensor operator==(const Tensor& value) const { return Eq(value); }
    [[nodiscard]] Tensor Eq(Scalar value) const;
    ...
    [[nodiscard]] Tensor Ne(const Tensor& value) const;
    Tensor operator!=(const Tensor& value) const { return Ne(value); }
    [[nodiscard]] Tensor Ne(Scalar value) const;
    ...
    /// each dimension.
    [[nodiscard]] std::vector<Tensor> NonZeroNumpy() const;
    ...
    /// tensor.
    [[nodiscard]] Tensor NonZero() const;
    ...
    [[nodiscard]] bool AllEqual(const Tensor& other) const;
    ...
    [[nodiscard]] Tensor IsClose(const Tensor& other,
                                 float rtol = 1e-5,
                                 float atol = 1e-8) const;
    ...
    /// strides and etc.
    [[nodiscard]] bool IsSame(const Tensor& other) const;
    ...
    [[nodiscard]] Tensor Solve(const Tensor& rhs) const;

    /// Solves the linear system AX = B with QR decomposition and returns X.
    /// A is a (m, n) matrix with m >= n.
    [[nodiscard]] Tensor LeastSquares(const Tensor& rhs) const;
    ...
    /// \return Tuple (P, L, U).
    [[nodiscard]] std::tuple<Tensor, Tensor, Tensor> LU(
            bool permute_l = false) const;
    ...
    [[nodiscard]] std::tuple<Tensor, Tensor> LUIpiv() const;
    ...
    [[nodiscard]] Tensor Triu(int diagonal = 0) const;
    ...
    [[nodiscard]] Tensor Tril(int diagonal = 0) const;
    ...
    [[nodiscard]] std::tuple<Tensor, Tensor> Triul(int diagonal = 0) const;

    /// Computes the matrix inversion of the square matrix *this with LU
    /// factorization and returns the result.
    [[nodiscard]] Tensor Inverse() const;

    /// Computes the matrix SVD decomposition A = U S VT and returns the result.
    /// Note VT (V transpose) is returned instead of V.
    [[nodiscard]] std::tuple<Tensor, Tensor, Tensor> SVD() const;

    /// Returns the size of the first dimension. If NumDims() == 0, an exception
    /// will be thrown.
    [[nodiscard]] inline int64_t GetLength() const {
        return GetShape().GetLength();
    }

    [[nodiscard]] inline SizeVector GetShape() const { return shape_; }

    [[nodiscard]] inline const SizeVector& GetShapeRef() const {
        return shape_;
    }

    [[nodiscard]] inline int64_t GetShape(int64_t dim) const {
        return shape_[shape_util::WrapDim(dim, NumDims())];
    }

    [[nodiscard]] inline SizeVector GetStrides() const { return strides_; }

    [[nodiscard]] inline const SizeVector& GetStridesRef() const {
        return strides_;
    }

    [[nodiscard]] inline int64_t GetStride(int64_t dim) const {
    ...
    [[nodiscard]] inline std::shared_ptr<Blob> GetBlob() const { return blob_; }

    [[nodiscard]] inline int64_t NumElements() const {
        return shape_.NumElements();
    }

    [[nodiscard]] inline int64_t NumDims() const { return shape_.size(); }
...
inline void Tensor::Fill(S v) {
    DISPATCH_DTYPE_TO_TEMPLATE_WITH_BOOL(GetDtype(), [&]() {
        auto casted_v = static_cast<scalar_t>(v);
